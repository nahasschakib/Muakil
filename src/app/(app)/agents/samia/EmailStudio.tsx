...
export default function EmailStudio({
  ...
  const [contexte, setContexte] = useState(
    previousOutput?.prospectSector
      ? `Secteur : ${previousOutput.prospectSector}`
      : "",
  );
  ...
  const inputClass =
    "w-full bg-[#1C1F2E] border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-sky-500/50";
  ...
  const objetPlaceholder: Record<string, string> = {
    ...
    newsletter:
      "ex: Nouveautés septembre, offre Aïd, lancement nouveau service...",
  };

  async function handleSave() {
    if (!result) return;
    const content = `## Email — ${result.typeOutput}\n\n${JSON.stringify(result, null, 2)}`;
    await saveLivrable({
      agentSlug: "samia",
      title: `Email — ${result.typeOutput}`,
      content,
    });
    setSaved(true);
  }
  ...
  return (
    <div className="min-h-screen bg-[#0F1117] text-white">
      <div className="border-b border-white/10 px-6 py-4 flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-sky-500/20 flex items-center justify-center text-lg">
          ✉️
        </div>
        <div>
          <h1 className="font-semibold text-white">Samia — Email Studio</h1>
          <p className="text-xs text-white/40">
            Emails · Relances · Newsletters
          </p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          {result && !saved && (
            <button
              onClick={handleSave}
              className="text-xs bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg font-medium transition-colors"
            >
              Enregistrer
            </button>
          )}
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className={labelClass}>
              {typeOutput === "newsletter"
                ? "Thème / actualité *"
                : "Objet / sujet *"}
            </label>
            <input
              className={inputClass}
              placeholder={objetPlaceholder[typeOutput]}
              value={objet}
