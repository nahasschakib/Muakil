...
type Platform = "Jumia" | "Hmizate" | "Site propre" | "Instagram Shop" | "WhatsApp Business";
...
const seasons: Season[] = ["Toute saison", "Ramadan", "Aïd", "Rentrée", "Été", "Hiver"];
const contentTypes: { id: ContentType; label: string; desc: string }[] = [
  { id: "fiche", label: "Fiche produit", desc: "Description + avantages + SEO" },
  { id: "script", label: "Script vidéo", desc: "Reels / TikTok / YouTube" },
  { id: "les deux", label: "Les deux", desc: "Fiche + script complets" },
];

const categories = [
  "Mode & Vêtements", "Beauté & Cosmétiques", "Électronique", "Maison & Déco",
  "Alimentation", "Sport & Fitness", "Enfants & Jouets", "Bijoux & Accessoires",
  "Auto & Moto", "Services",
];
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/agents/yasmine/generate", {
        ...
        body: JSON.stringify({ productName, category, price, platform, season, contentType, description, targetAudience }),
      });
      ...
      <div className="border-b border-[#2A2D3E] bg-[#1C1F2E]/60 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">EcomStudio — Fiches produits & scripts vidéo pour le marché marocain</p>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <section className="space-y-3">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Plateforme</h2>
            <div className="space-y-2">
              {platforms.map((p) => (
                <button key={p.id} onClick={() => setPlatform(p.id)}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                    platform === p.id ? "border-rose-500 bg-rose-500/10" : "border-[#2A2D3E] bg-[#1C1F2E] hover:border-[#3A3D4E]"
                  }`}>
                  ...
                  {platform === p.id && <span className="ml-auto w-2 h-2 rounded-full bg-rose-400" />}
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-2">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Type de contenu</h2>
              ...
              {contentTypes.map((t) => (
                <button key={t.id} onClick={() => setContentType(t.id)}
                  className={`w-full text-left px-3 py-2 rounded-lg border text-xs font-medium transition-all ${
                    contentType === t.id ? "border-rose-500 bg-rose-500/20 text-rose-300" : "border-[#2A2D3E] text-gray-400 hover:border-gray-500"
                  }`}>
                  {t.label} <span className="text-gray-500 font-normal">— {t.desc}</span>
                </button>
              ))}
            </div>
          </section>
          ...
          {/* Produit */}
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Nom du produit</label>
          ...
          {/* Catégorie */}
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Catégorie</label>
            <div className="flex flex-wrap gap-1.5">
              {categories.map((c) => (
                <button key={c} onClick={() => setCategory(c)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-all ${
                    category === c ? "border-rose-500 bg-rose-500/20 text-rose-300" : "border-[#2A2D3E] text-gray-400 hover:border-gray-500"
                  }`}>
                  {c}
                </button>
              ))}
          ...
          {/* Prix */}
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Prix (MAD)</label>
          ...
          {/* Saison */}
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Saison / Occasion</label>
            <div className="flex flex-wrap gap-2">
              {seasons.map((s) => (
                <button key={s} onClick={() => setSeason(s)}
                  className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${
                    season === s ? "border-rose-500 bg-rose-500/20 text-rose-300" : "border-[#2A2D3E] text-gray-400 hover:border-gray-500"
                  }`}>
                  {s}
                </button>
              ))}
          ...
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
              Description <span className="text-gray-600 font-normal normal-case">(optionnel)</span>
            </label>
          ...
          <section className="space-y-2">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
              Audience <span className="text-gray-600 font-normal normal-case">(optionnel)</span>
            </label>
          ...
          <button onClick={handleGenerate} disabled={loading || !productName.trim() || !price.trim()}
        ...
        <div className="lg:col-span-3">
          {content ? (
            ...
            <div className="h-full min-h-64 flex flex-col items-center justify-center text-center rounded-2xl border border-dashed border-[#2A2D3E] p-12">
              <div className="w-14 h-14 rounded-2xl bg-[#1C1F2E] flex items-center justify-center text-2xl mb-4">🛍️</div>
              <p className="text-gray-400 text-sm font-medium">Votre fiche produit apparaîtra ici</p>
              <p className="text-gray-600 text-xs mt-1">Décrivez votre produit et choisissez la plateforme</p>
            </div>
          )}
        </div>
      </div>
