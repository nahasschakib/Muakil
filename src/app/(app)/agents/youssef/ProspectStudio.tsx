    ...
    <div className="min-h-screen bg-[#0F1117] text-white">
      ...
      <div className="border-b border-[#2A2D3E] bg-[#1C1F2E]/60 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          {sequence && !saved && (
            <button onClick={handleSave} className="ml-auto text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
              Enregistrer la séquence
            </button>
          )}
          {saved && <span className="ml-auto text-xs text-emerald-400 font-medium">✓ Enregistré dans les livrables</span>}
          {copied && <span className="ml-auto text-xs text-blue-400 font-medium">✓ Copié !</span>}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <button
            ...
            disabled={loading || !sector.trim() || !prospectRole.trim()}
