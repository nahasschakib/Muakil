          ...
          <div className="ml-auto flex items-center gap-2">
            {invoice && (
              <>
                ...
                {qrData && <span className="text-xs text-emerald-400 font-medium">✓ QR Code généré</span>}
                {!saved ? (
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-2 space-y-5">
          <section className="space-y-3">
            ...
            <input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="N° Facture"
          ...
          <section className="space-y-2">
            ...
            {lines.map((line, i) => (
              <div key={i} className="bg-[#1C1F2E] border border-[#2A2D3E] rounded-xl p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input type="number" min="1" value={line.qty} onChange={(e) => updateLine(i, "qty", Number(e.target.value))} placeholder="Qté"
                  ...
                  <input type="number" min="0" value={line.unitPrice} onChange={(e) => updateLine(i, "unitPrice", Number(e.target.value))} placeholder="Prix unitaire HT"
                    className="w-full bg-[#0F1117] border border-[#2A2D3E] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-rose-500" />
                </div>
          ...
          <section className="space-y-3">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <div onClick={() => setWithholding(!withholding)}
